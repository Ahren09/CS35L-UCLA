...
void* func(void* ID)
{
    ...
    for(int px=id; px<width; px+=nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            //$$TEST: Plug this into upper parts
            int index;
            for(index=0;index<3;index++)
                ALL_SCALED_COLOR[px][py][index]=scaled_color[index];
...
main( int argc, char **argv )
{
    ...
    int m, n;
    ALL_SCALED_COLOR = (float***)malloc(sizeof(float**) * width);
    for(m=0;m<width;m++)
    {
        ALL_SCALED_COLOR[m] = (float**)malloc(sizeof(float*) * height);
        for(n=0;n<height;n++)
        {
            ALL_SCALED_COLOR[m][n]=(float*)malloc(sizeof(float) * 3);
        }
    }
    ...
    //NOTICE: For multithreading,
    //we can NOT plug in a temporary variable as parameter
    ...
    for(i = 0; i<nthreads; i++)
     {
       ...
       {
           fprintf(stderr,"Error: Join failure");
           exit(1);
       }
     }
    ...
    for(m=0;m<width;m++)
    {
        ...
        for(n=0;n<height;n++)
        {
            printf( "%.0f %.0f %.0f\n",ALL_SCALED_COLOR[m][n][0], ALL_SCALED_COLOR[m][n][1],
                   ALL_SCALED_COLOR[m][n][2]);
        }
        printf("\n");
    }
